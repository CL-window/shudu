  ...
  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      if (a[i][j] !== 0) {
        ...
        table[i][j].innerHTML = '<input type="button" class="active" style="cursor: pointer" onclick="clickItem(' + i + ',' + j + ');"/>';
        // var index = 3 * parseInt(i / 3) + parseInt(j / 3) + 1;
        // table[i][j].innerHTML = '<input type="button" class="color'+index+'"  onclick="clickItem(' + i + ',' + j + ');"/>';
      }
    }
  }
}
...
function showAnswer() {
  showTable(answer);
  showToast("2秒后答案自动消失", 1000);
  setTimeout("showTable(sudoku)", 2000);
}

function checkInputChange(e) {
  checkInput = e.checked;
}
...
function onBiaoJiClick(e) {
  e.className = 'biaoji touchhover';
  setTimeout(() => {
    e.className = 'biaoji';
  }, 200);
  lastItem.updateTag();
...
class ClickItem {
  ...
  reset() {
    if (this.element) {
      // var index = 3 * parseInt(this.x / 3) + parseInt(this.y / 3) + 1;
      // this.element.className = 'color' + index;
      var name = this.element.className;
      var newName;
      if (name.indexOf("cur_tag") != -1) {
        if (sudoku[this.x][this.y] != 0) {
          newName = 'has_edit cur_tag';
        } else {
          newName = 'active cur_tag';
        }
      } else {
        if (sudoku[this.x][this.y] != 0) {
          newName = 'has_edit';
  ...
  update(i, j, e) {
    ...
    var name = this.element.className;
    if (name.indexOf("cur_tag") != -1) {
      this.element.className = 'cur_edit cur_tag';
    } else {
  ...
  updateInfo(value) {
    if (this.element) {
      ...
      var legal = true;
      if (checkInput) {
        legal = check(this.x, this.y, value);
      }
      if (legal) {
        sudoku[this.x][this.y] = value;
        // this.reset();
